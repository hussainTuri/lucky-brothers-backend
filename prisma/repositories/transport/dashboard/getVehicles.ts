...
export async function getTransportVehiclesDashboard(
  ...
  // Fetch vehicles page and total count
  ...
  if (vehicleIds.length === 0) {
    ...
    const totalBalance = totalLoanAmount + totalExpenses + totalCustomerPayments;
  ...
  const rows = vehicles.map((v) => {
    ...
    const balance = loanAmount + expenses + customerPayments;
